...
function TechnologyStartUpPage() {
  ...
  useEffect(() => {
    const fetchTechnologyStartUps = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:3001/api/business-type?type=technology');
        ...
        <div className="bg-white text-black py-16">
          <div className="container mx-auto px-4 w-fit">
            <div className="text-center mb-8">
              <h1 className="text-[48px] font-bold">Technology Startups</h1>
            </div>

            {/* SortDropdown component */}
            <div className="flex justify-between items-center mb-8">
              <SortOption sortOption={sortOption} setSortOption={setSortOption} />
              <span className="text-[18px] text-gray-500">Found {sortedDeals.length} technology startup(s)</span>
